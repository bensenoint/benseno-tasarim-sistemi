...
function UsersScreen({ currentUser }) {
  ...
  const headSt = { padding: '8px 12px', font: '600 11px/1 var(--font-sans)', color: 'var(--ink-4)', textTransform: 'uppercase', letterSpacing: '0.06em', background: 'var(--paper)', borderBottom: '1px solid var(--line)' };

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <div style={{ font: 'italic 500 26px/1.1 var(--font-display)', color: 'var(--ink)' }}>Kullanıcılar</div>
        ...
        <button onClick={() => { setShowForm(v => !v); setFormError(''); }}
          style={{ padding: '8px 14px', border: '1px solid var(--ember)', borderRadius: 6, background: 'transparent', color: 'var(--ember)', font: '600 13px/1 var(--font-sans)', cursor: 'pointer' }}>
          {showForm ? 'İptal' : '+ Kullanıcı ekle'}
        </button>
      </div>

      {successMsg && (
        <div onClick={() => setSuccessMsg('')} style={{ marginBottom: 16, padding: '10px 14px', borderRadius: 0, border: '1px solid var(--line)', borderLeft: '2px solid var(--prio-green, #1B7A37)', background: 'transparent', color: 'var(--prio-green, #1B7A37)', font: '500 13px/1 var(--font-sans)', cursor: 'pointer' }}>
          ✓ {successMsg}
        </div>
      )}

      {showForm && (
        <form onSubmit={handleCreate} style={{ marginBottom: 24, padding: 20, border: '1px solid var(--line)', borderRadius: 0, background: 'transparent' }}>
          <div style={{ font: 'italic 500 18px/1.15 var(--font-display)', color: 'var(--ink)', marginBottom: 16 }}>Yeni kullanıcı</div>
            ...
            {[['slack_id', 'Slack ID', 'U0123…', 'text'], ['name', 'İsim', 'Görkem K.', 'text'], ['password', 'Geçici şifre', '', 'password']].map(([key, label, ph, type]) => (
              <div key={key}>
                ...
                <input type={type} value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                  ...
                  style={{ width: '100%', padding: '8px 10px', border: '1px solid var(--line)', borderRadius: 6, fontSize: 13, boxSizing: 'border-box', background: 'var(--paper)', color: 'var(--ink)' }}/>
              </div>
            ))}
            <div>
              <div style={{ font: '600 11px/1 var(--font-sans)', color: 'var(--ink-3)', marginBottom: 5 }}>Rol</div>
              <select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}
                style={{ width: '100%', padding: '8px 10px', border: '1px solid var(--line)', borderRadius: 6, fontSize: 13, background: 'var(--paper)', color: 'var(--ink)' }}>
          ...
          <button type="submit" disabled={formLoading}
            style={{ padding: '9px 18px', border: 0, borderRadius: 6, background: formLoading ? 'var(--line)' : 'var(--navy, #24479E)', color: '#fff', font: '600 13px/1 var(--font-sans)', cursor: 'pointer' }}>
                  ...
                  <td style={cellSt}>
                    {patchTarget?.id === u.id && patchTarget.field === 'role' ? (
                      ...
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                        <I.Dot size={6} color={u.role === 'admin' ? 'var(--navy, #24479E)' : 'var(--ink-4)'}/>
                        <span style={{ font: '500 12px/1 var(--font-sans)', color: u.role === 'admin' ? 'var(--ink-2)' : 'var(--ink-3)' }}>{u.role === 'admin' ? 'Admin' : 'Üye'}</span>
                      </span>
                    )}
                  </td>
