...
function TakvimScreen() {
  ...
  const [f, setF] = React.useState({ gun: "", ad: "", yarim: false });
  ...
  const ekle = async () => {
    ...
    setSt("kaydediliyor");
    try { await apiYaz("POST", "/api/tatiller", f); setF({ gun: "", ad: "", yarim: false }); setSt(null); yukle(); if (window.bnsRefresh) window.bnsRefresh(); }
    ...
    <div style={{ maxWidth: 760, display: "flex", flexDirection: "column", gap: "var(--grid-gap)" }}>
      <div>
        ...
        <div style={{ font: "400 13px/1.5 var(--font-sans)", color: "var(--ink-3)", marginTop: 4 }}>
          Resmî tatiller — kapasite bölünmesi, çalışma süresi ölçümü, kalan iş günü ve fatura takip günü bu takvime göre hesaplanır. Hafta sonları otomatik; yarım gün = 09:00-13:00 mesai, 0,5 gün kapasite.</div>
      </div>
