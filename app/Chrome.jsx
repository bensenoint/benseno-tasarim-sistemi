...
function Header({ user, viewMode, setViewMode, theme, setTheme, onOpenPalette, onNewBrief, defaultUsers }) {
  ...
  return (
    <header style={{
      ...
      {/* Search — mobile'da icon, desktop'ta sidebar'da olduğu için burada sadece mobilde */}
      {isMobile && (
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        {collapsed ? (
          <button onClick={onOpenPalette} style={{width:32, height:32, display:"flex", alignItems:"center", justifyContent:"center", border:"1px solid var(--line)", borderRadius:7, background:"var(--paper-2)", color:"var(--ink-4)", cursor:"pointer", flexShrink:0}}>
            <I.Search size={13}/>
          </button>
        ) : (
          <button onClick={onOpenPalette} style={{flex:1, minWidth:0, display:"flex", alignItems:"center", gap:7, padding:"6px 9px", border:"1px solid var(--line)", borderRadius:7, background:"var(--paper-2)", color:"var(--ink-4)", cursor:"pointer", font:"400 12px/1 var(--font-sans)", textAlign:"left"}}
